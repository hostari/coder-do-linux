...
variable "step3_do_token" {
  type = string
  description = "Enter DigitalOcean PAT:"
  sensitive = true
}
...
variable "droplet_size" {
  ...
  default     = "s-1vcpu-1gb"
  validation {
    condition     = contains(["s-1vcpu-1gb", "s-1vcpu-2gb", "s-2vcpu-2gb", "s-2vcpu-4gb", "s-4vcpu-8gb", "s-4vcpu-8gb-amd"], var.droplet_size)
    error_message = "Value must be s-1vcpu-1gb, s-1vcpu-2gb, s-2vcpu-2gb, s-2vcpu-4gb, s-4vcpu-8gb or s-4vcpu-8gb-amd."
  }
}
